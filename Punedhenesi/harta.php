...
<html>
<head>
 <?php echo '<a href="MenaxhoPun.php">Prapa</a>'; ?>
<title>Google Map</title>
<script type="text/javascript" src="../js/jquery-1.10.2.min.js"></script>
<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?sensor=flase"></script>
<script type="text/javascript">
$(document).ready(function() {

	var mapCenter = new google.maps.LatLng(42.672421000000000000 ,21.164538999999990000); //Google map Coordinates
	var map;
	...
	function map_initialize()
	{
			...
			google.maps.event.addListener(map, 'rightclick', function(event) {
				...
				'<label for="pName"><span>Emri i vendit :</span><input type="text" name="pName" class="save-name" placeholder="Shkruani vendin" maxlength="40" /></label>'+
				'<label for="pDesc"><span>Pershkrimi :</span><textarea name="pDesc" class="save-desc" placeholder="Shkruani adresen" maxlength="150"></textarea></label>'+
				'<label for="pType"><span>Tipi i vendit :</span> <select name="pType" class="save-type"><option value="restaurant">Restaurant</option><option value="bar">Lagje</option>'+
				'<option value="house">Rruge</option></select></label>'+
				'</form>'+
				'</div></p><button name="save-marker" class="save-marker">Ruaj te dhenat</button>';
	...
	{	  	  		  
		...
		MapDesc+ 
		'</span><button name="remove-marker" class="remove-marker" title="Remove Marker">Fshij markerin</button>'+
		'</div></div>');	
		...
		if(typeof saveBtn !== 'undefined') //continue only when save button is present
		{
			...
			google.maps.event.addDomListener(saveBtn, "click", function(event) {
				...
				if(mName =='' || mDesc =='')
				{
					alert("Shkruani emrin dhe pershkrimin!");
				}else{
...
<body>             
<h1 class="heading">Google Maps</h1>
<div align="center">Vendos markerin me klikim te djathte</div>
